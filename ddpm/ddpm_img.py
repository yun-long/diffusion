...
# ---------------------------
# 2) Noise schedule (Cosine; replace with your linear if desired)
# ---------------------------
...
# ---------------------------
# 3) Tiny U-Net + sinusoidal t-embedding
# ---------------------------
...
# ---------------------------
# 4) Training loop (unchanged, but we add grad clip + EMA)
# ---------------------------
epochs = 20  # increase to 50+ for sharper digits
model.train()
for epoch in range(epochs):
    for x0, _ in dataloader:
        ...
        alpha_bar_t = alphas_bar[t-1].view(B, 1, 1, 1)
...
def sample_with_ema(model, n_samples=16):
    ...
    for t in reversed(range(1, T+1)):
        ...
        eps_theta = tmp(x_t, torch.full((n_samples,), t, device=device))
...
for i, ax in enumerate(axes.flatten()):
    ...
    ax.axis("off")
plt.tight_layout()
plt.savefig("mnist_samples/generated_unet.png")
plt.show()

print("Saved samples to mnist_samples/generated_unet.png")
